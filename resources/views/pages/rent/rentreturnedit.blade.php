<form action="/rentcar-returneditupdate/{{ $rent->id }}" method="post" enctype="multipart/form-data">
    @csrf
    ...
    <div class="input-group input-group-outline mt-3 focused is-focused">
        ...
        <select name="car_id" id="car_id" class="form-control border border-2 p-2">
            @foreach ($car as $item)
            <option value="{{ $item->id }}" {{ $item->id == $rent->car_id ? 'selected' : '' }}>{{ $item->nama_mobil }}</option>
            @endforeach
        </select>
    </div>
    <div class="input-group input-group-outline mt-3 focused is-focused">
